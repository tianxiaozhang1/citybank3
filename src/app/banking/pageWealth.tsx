...
import { ChevronDown, CreditCard, DollarSign, TrendingUp, ArrowDownCircle, ArrowUpCircle, Home, Briefcase, ShoppingBag, MoreHorizontal, Car, Send, FileText, FilePlus, LogOut, User, Bell, Settings, LifeBuoy } from 'lucide-react';
import React, { useState, useEffect } from 'react';
import NextLink from 'next/link'; // Renamed to avoid conflict if Link is imported from lucide or elsewhere

import { inter, lora, aldrich, pt_sans_narrow } from '../../fonts'; // Updated font import
...
import Header from '../../components/Header'; // Assuming Header is in components folder relative to app directory
...
const accountsData: Account[] = [
  { id: 'acc1', name: 'Everyday Chequing', type: 'Chequing', balance: 12530.75, currency: 'CAD', bgColor: 'bg-gradient-to-br from-[#106898] to-[#06436F]', textColor: 'text-white', icon: Home },
...
const summaryItemsData: SummaryItem[] = [
  ...
  { title: 'Portfolio Value', value: formatCurrency(47300.50, 'CAD'), icon: TrendingUp, bgColor: 'bg-[#32302f]', textColor: 'text-white', href: '/investment' },
...
const AccountCard: React.FC<{ account: Account; index: number }> = ({ account, index }) => {
  ...
  if (typeof window !== 'undefined' && window.innerWidth < 640) { // Simple mobile detection (adjust as needed)
    ...
    // cardStyle = `absolute bottom-0 left-0 w-full h-full rounded-xl p-4 shadow-lg transition-all duration-300 ease-in-out ${account.bgColor}`;

    // cardStyle = `absolute bottom-${index * 20}% left-0 w-full h-full rounded-xl p-4 shadow-lg transition-all duration-300 ease-in-out ${account.bgColor}`;

    // cardStyle = `absolute bottom-${index * 20}% left-0 w-full h-full rounded-xl p-4 shadow-lg transition-all duration-300 ease-in-out ${account.bgColor}`;
    // cardStyle = `relative bottom-${index * 20}% left-0 w-full h-full rounded-xl p-4 shadow-lg transition-all duration-300 ease-in-out ${account.bgColor}`;
    // cardStyle = `absolute bottom-0 left-0 w-full rounded-xl p-4 shadow-lg transition-all duration-300 ease-in-out ${account.bgColor}`;
    // cardStyle = `relative bottom-${index * 20}% left-0 w-full rounded-xl p-4 shadow-lg transition-all duration-300 ease-in-out ${account.bgColor}`;
    // cardStyle = `absolute bottom-${index * 20}% left-0 w-full h-full rounded-xl p-4 shadow-lg transition-all duration-300 ease-in-out ${account.bgColor}`;
...
const SummaryCard: React.FC<{ item: SummaryItem }> = ({ item }) => {
  ...
  return (
    <NextLink href={item.href} className={`rounded-xl p-6 shadow-md flex items-center space-x-4 hover:shadow-lg transition-shadow duration-300 hover:-translate-y-0.5 ${item.bgColor} ${item.textColor || 'text-white'}`}>
        <div className={`p-3 rounded-lg bg-white bg-opacity-25 text-white`}> {/* Ensured icon container is translucent white, icon color from parent */}
        ...
        <main className="container mx-auto p-4 sm:p-6 lg:p-8">
          ...
          {summaryItemsData.length > 0 && (
            <section id="summary-overview" className="mb-12">
              <h2 className="text-2xl font-semibold text-gray-700 mb-6">Other Holdings & Services</h2>
