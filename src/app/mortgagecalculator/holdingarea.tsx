...
function MortgageCalculator() {
    ...
    const calculateMortgage = () => {
        ...
        // HIGHLIGHT START: Get the number of payments per year based on frequency
        ...
        const compoundingPeriodsPerYear = 2; // Semi-annual compounding

        // Calculate the effective annual rate
        const effectiveAnnualRate = Math.pow((1 + semiAnnualRate), compoundingPeriodsPerYear) - 1;

        // Calculate the equivalent rate per payment period
        const ratePerPeriod = Math.pow((1 + effectiveAnnualRate), (1 / paymentsPerYear)) - 1;
        // HIGHLIGHT END
        ...
        if (ratePerPeriod === 0) {
            ...
            setMonthlyPayment(paymentValue); // This is the payment *per frequency period*
        }
                        ...
                        <div className=''> {/* Added gap and flex-col/md:flex-row for responsiveness */}
                            ...
                            <div className='w-full flex justify-center items-center border-2 border-stone-200 rounded-2xl mt-8 p-8 lg:min-h-[150px]'> 
                                {monthlyPayment !== null ? (
                                    ...
                                    <p className='text-4xl'>${monthlyPayment.toFixed(2)}</p>
                                    </div>
                                ) : (
